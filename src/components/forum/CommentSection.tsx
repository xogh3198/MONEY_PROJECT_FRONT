...
export default function CommentSection({ postId }: Props) {
  ...
  const submitComment = async (content: string, parentCommentId?: string | null) => {
    ...
    if (!token) { alert('Î°úÍ∑∏?∏Ïù¥ ?ÑÏöî?©Îãà??); return; }
  ...
  if (loading) return <div className="py-4 text-text-secondary text-sm animate-pulse">?ìÍ? Î∂àÎü¨?§Îäî Ï§?..</div>;

  return (
    <div className="pt-4">
      <h4 className="text-sm font-bold text-text-primary mb-3">?ìÍ? {comments.length}Í∞?/h4>
      <form onSubmit={handleSubmit} className="flex gap-2 mb-4">
        <input
          type="text" value={input} onChange={(e) => setInput(e.target.value)}
          placeholder="?òÍ≤¨???®Í≤®Î≥¥ÏÑ∏??.." maxLength={1000}
    ...
    <div>
      <div className="bg-card rounded-md p-3 border border-border">
        ...
        <button onClick={() => setReplyTo(replyTo === comment.id ? null : comment.id)}
          className="text-[11px] text-text-secondary mt-1 hover:text-accent">?µÍ?</button>
      </div>
      {replyTo === comment.id && (
        <form onSubmit={handleReply} className="flex gap-2 mt-1 ml-4">
          <input type="text" value={replyInput} onChange={(e) => setReplyInput(e.target.value)}
            placeholder="?µÍ? ?ÖÎ†•..." maxLength={1000}
          ...
          <button type="submit" className="px-3 py-1.5 text-xs font-medium text-black bg-accent rounded hover:opacity-90">?±Î°ù</button>
        </form>
      )}
      {replies.length > 0 && (
...
function timeAgo(dateStr: string): string {
  ...
  if (diff < 60) return `${diff}Î∂???;
  if (diff < 1440) return `${Math.floor(diff / 60)}?úÍ∞Ñ ??;
