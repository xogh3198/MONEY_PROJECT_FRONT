...
export default function NewsArticlePage({ params }: { params: { id: string } }) {
  ...
  if (loading) {
    return <div className="py-16 text-center text-sm text-text-secondary">기사를 불러오는 중입니다.</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <Link href="/forum" className="mb-4 inline-flex text-xs text-text-secondary hover:text-text-primary">
        ← 뉴스 목록
      </Link>
      ...
      <article className="mb-6 rounded-xl border border-border bg-card p-5 md:p-7">
        ...
        <h1 className="mb-4 text-xl font-bold leading-snug text-text-primary md:text-2xl">
          {article.title}
        </h1>
        ...
        <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
          ...
          {article.sourceUrl && article.sourceUrl !== '#' && !article.sourceUrl.includes('example.com') && (
            <a
              ...
              className="rounded-lg border border-border px-4 py-2 text-sm text-accent-blue hover:bg-bg/40"
            >
              원문 보기 ↗
            </a>
          )}
      ...
      <p className="mt-8 text-center text-[11px] text-text-secondary">
        본 정보와 댓글은 투자 조언이 아니며, 투자 판단의 책임은 사용자에게 있습니다.
      </p>
    </div>
  );
}
