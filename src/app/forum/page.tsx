...
export default function ForumPage() {
  ...
  return (
    <div>
      <ForumTabs />
      <div className="mb-5">
        <h1 className="text-xl font-bold">ê²½ì œ?´ìŠ¤</h1>
        <div className="flex items-center gap-2 mt-1">
          <p className="text-xs text-text-secondary">?¤ì‹œê°?ê²½ì œ ?´ìŠ¤?€ ?œì¥ ë¶„ì„</p>
          {tab === 'realtime' && (
      ...
      {loading && articles.length === 0 ? (
        <div className="text-center py-12 text-text-secondary text-sm animate-pulse">ë¶ˆëŸ¬?¤ëŠ” ì¤?..</div>
      ) : articles.length === 0 ? (
        <div className="text-center py-12 text-text-secondary text-sm">?´ìŠ¤ê°€ ?†ìŠµ?ˆë‹¤</div>
      ) : (
      ...
      {loading && articles.length > 0 && (
        <div className="text-center py-4 text-text-secondary text-xs animate-pulse">ë¡œë”© ì¤?..</div>
      )}
...
function ArticleRow({ article, rank, showRank }: { article: Article; rank: number; showRank: boolean }) {
  const sentimentLabel = {
    POSITIVE: { text: 'ê¸ì •', color: 'text-[#f85149] bg-[#f85149]/10' },
    NEGATIVE: { text: 'ë¶€??, color: 'text-[#58a6ff] bg-[#58a6ff]/10' },
    NEUTRAL: { text: 'ì¤‘ë¦½', color: 'text-text-secondary bg-border/50' },
  }[article.sentiment] || { text: 'ì¤‘ë¦½', color: 'text-text-secondary bg-border/50' };

  const categoryMap: Record<string, string> = {
    DOMESTIC: 'êµ?‚´ì¦ì‹œ', OVERSEAS: '?´ì™¸ì¦ì‹œ', FOREX: '?˜ìœ¨', RATE: 'ê¸ˆë¦¬', CRYPTO: '?”í˜¸?”í',
  };
